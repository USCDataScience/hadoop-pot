...
public class PoT {
	...
	public static void main(String[] args) {
		...
		/*Path file = Paths.get("C:\\data\\memex\\0046e75e41422a2f4f74d64fc921273f_1-d347ad4b9e9b61080efc53f9711df8.mp4");
		
		ArrayList<double[][]> multi_series = new ArrayList<double[][]>();
		multi_series.add(getOpticalTimeSeries(file, 5, 5, 8));
		//multi_series.add(getGradientTimeSeries(file, 5, 5, 8));
		
		saveVectors(multi_series.get(0), Paths.get("outout6.txt"));
		
		ArrayList<double[]> tws = getTemporalWindows(1);
		
		FeatureVector fv = new FeatureVector();
		
		for(int i=0;i<multi_series.size();i++) {
			ArrayList<Double> one_feature = computeFeaturesFromSeries(multi_series.get(i), tws, 1);
			one_feature.addAll(computeFeaturesFromSeries(multi_series.get(i), tws, 2));
			one_feature.addAll(computeFeaturesFromSeries(multi_series.get(i), tws, 5));
			
			normalizeFeatureL1(one_feature);
			
			fv.feature.add(one_feature);
		}*/
	...
	public static void evaluateSimilarity(ArrayList<Path> files, int save_mode) {
		...
		for(int k=0;k<files.size();k++) {
			System.out.println(files.get(k).toString());
			...
			for(int i=0;i<multi_series.size();i++) {
				...
				//if(k==4) for(int j=0;j<one_feature.size();j++) System.out.format("%f ", one_feature.get(j));
			}
			System.out.println("");
			
			
			// normalize
			//for(int i=0;i<fv.numDim();i++) normalizeFeatureL1(fv.feature.get(i));
			fv_list.add(fv);
		}
		
		// feature vector similarity measure
		double[] mean_dists = new double[fv_list.get(0).numDim()];
		for(int i=0;i<fv_list.get(0).numDim();i++) mean_dists[i] = meanChiSquareDistances(fv_list, i);
		
		for(int i=0;i<fv_list.get(0).numDim();i++) System.out.format("%f ", mean_dists[i]);
		System.out.println("");
		
		try(FileOutputStream  fos = new FileOutputStream("similarity.txt");BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(fos));) {
			for (int i = 0; i < fv_list.size(); i++) {
				System.out.format("%d ", i);
	
				for (int j = 0; j < fv_list.size(); j++) {
					double sim = kernelDistance(fv_list.get(i), fv_list.get(j), mean_dists);
	...
	public static double[][] getOpticalTimeSeries(Path filename, int w_d, int h_d, int o_d)
	{	
		ArrayList<double[][][]> hists = getOpticalHistograms(filename, w_d, h_d, o_d);
	...
	static ArrayList<double[][][]> getOpticalHistograms(Path filename, int w_d, int h_d, int o_d)
	{
		...
		if(!capture.isOpened())  System.out.println("video file not opened.");
		...
		// computing a list of histogram of optical flows (i.e. a list of 5*5*8 arrays)
		for (int frame_index=0;;frame_index++) {
			...
			if (original_frame.empty()) {
				...
				// resizing the captured frame and converting it to the gray scale image.
	...
	static int opticalFlowType(Point fxy, int dim)
	{
		...
		/*if(degree < -3*Math.PI/4) type = 0;
		else if(degree < -1*Math.PI/2) type = 1;
		else if(degree < -1*Math.PI/4) type = 2;
		else if(degree < 0) type = 3;
		else if(degree < Math.PI/4) type = 4;
		else if(degree < Math.PI/2) type = 5;
		else if(degree < 3*Math.PI/4) type = 6;
		else type = 7;*/
	...
	public static double[][] getGradientTimeSeries(Path filename, int w_d, int h_d, int o_d)
	{
		ArrayList<double[][][]> hists = getGradientHistograms(filename, w_d, h_d, o_d);
		double[][] vectors = new double[hists.size()][];
	...
	static ArrayList<double[][][]> getGradientHistograms(Path filename, int w_d, int h_d, int o_d)
	{
		...
		// initializing a list of histogram of gradients (i.e. a list of s*s*9 arrays)
		for (int i=0;;i++) {
			// capturing the video images
			capture.read(original_frame);
			if (original_frame.empty()) {
